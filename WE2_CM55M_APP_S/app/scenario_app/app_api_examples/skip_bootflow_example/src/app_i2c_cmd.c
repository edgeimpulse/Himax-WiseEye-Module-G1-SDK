...
static int32_t priv_i2c_cmd_handler(USE_DW_IIC_SLV_E device_id)
{
    ...
    switch (feature_type)//Any modify need sync with i2ccomm_task
    {
        ...
        case I2CCOMM_FEATURE_SENSOR_REG_ACCESS:
            ...
            i2ccomm_cmd_process_sensor_reg_rw(gRead_buf);
            board_delay_ms(200);//delay some times for sensor to get/set the register.
            break;
...
#ifdef OS_FREERTOS
    if (xTaskCreate(i2ccomm_task, "i2ccomm_task", configMINIMAL_STACK_SIZE + 256, (void *)&gI2CCOMM_cfg[iic_id], (configMAX_PRIORITIES - 1), &i2ccomm_task_handle) != pdPASS) {
...
static void i2ccomm_task(void *par)
{
    ...
    while (1)
    {
        ...
        /* Fetch and Process I2C commands from I2C Slave interface. */
        priv_i2c_cmd_handler(device_id);
    }
